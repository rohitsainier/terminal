...
- **Install script:** `install.sh` detects OS/arch, downloads the right binary from GitHub Releases, installs to `/usr/local/bin` (or `~/.local/bin` as fallback). Usage: `curl -fsSL https://raw.githubusercontent.com/rohitsainier/terminal/main/install.sh | sh`
